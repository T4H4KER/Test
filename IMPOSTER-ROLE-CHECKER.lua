...
local function IsPlayerDead(player)
	...
	while true do
		...
		if (FlatIdent_35A31 == 1) then
			...
			publicStates = player:FindFirstChild("PublicStates");
			FlatIdent_35A31 = 2;
		end
	end
end
...
local function getPlayerRoleData(player)
	...
	while true do
		if (FlatIdent_8ABD6 == 2) then
			containers = {player,player:FindFirstChild("States"),player:FindFirstChild("PublicStates")};
...
local function updateUI()
	...
	if not isMinimized then
		local targetContentHeight = math.clamp(totalY + 40, 70, 380);
